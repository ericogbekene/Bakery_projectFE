...
export default async function Page() {
  ...
  try {
    const data = await productService.getProducts({ category: "loaves" });
    loaves = data.results ?? [];
  } catch (error) {
      ...
      <Container className="my-8 mt-12 grid grid-cols-1 gap-8 md:grid-cols-2 lg:my-16 lg:mt-16 lg:grid-cols-3">
        {loaves.length === 0 ? (
          <div className="col-span-3 py-16 text-center text-gray-400">
            <p className="text-lg">No loaves available at the moment.</p>
          </div>
        ) : (
          loaves.map((item) => (
            <div key={item.id} className="shadow-md">
              ...
              <div className="text-dark-text px-3 py-4 text-center">
                ...
                >
                  <Link
                    href={`/menu/loaves/order?title=${encodeURIComponent(item.name)}&id=${item.id}`}
                  >
                    Order now
                  </Link>
                </Button>
