...
const PastriesPage = () => {
  ...
  const [loafProducts, setLoafProducts] = useState<Product[]>([]);

  useEffect(() => {
    setBackgroundImage(Cakes.src);
    const loaf = useProductStore.getState().getProductsByType("loaves");
    setLoafProducts(loaf);
  }, []);
